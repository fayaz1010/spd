    ...
    if (type === 'full' || type === 'database') {
      ...
      try {
        ...
        results.database = {
          ...
          file: backupFile,
          size: stats.size,
          sizeM B: (stats.size / 1024 / 1024).toFixed(2),
        };
        ...
        if (compress) {
          ...
          results.database.compressed = true;
          results.database.file = compressedFile;
          results.database.size = compressedStats.size;
          results.database.sizeMB = (compressedStats.size / 1024 / 1024).toFixed(2);
        }
      } catch (error: any) {
        results.database = {
          success: false,
