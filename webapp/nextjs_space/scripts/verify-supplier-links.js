...
async function verifySupplierLinks() {
  ...
  const productsWithInactiveSuppliers = [];
  ...
  if (!defaultSupplier) {
    ...
    defaultSupplier = await prisma.supplier.create({
      data: {
        ...
        contactEmail: 'admin@sundirect.com.au',
        isActive: true,
        updatedAt: new Date(),
      },
    });
  ...
  const totalPanels = finalProducts.filter(p => p.productType === 'PANEL').length;
  const totalBatteries = finalProducts.filter(p => p.productType === 'BATTERY').length;
  const totalInverters = finalProducts.filter(p => p.productType === 'INVERTER').length;

  console.log(`✅ Panels with active suppliers: ${panelsWithSuppliers}/${totalPanels}`);
  console.log(`✅ Batteries with active suppliers: ${batteriesWithSuppliers}/${totalBatteries}`);
  console.log(`✅ Inverters with active suppliers: ${invertersWithSuppliers}/${totalInverters}`);
